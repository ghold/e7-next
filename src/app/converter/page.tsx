...
export default function ConverterPage() {
  ...
  const updateSubstat = (index: number, field: keyof SubstatConfig, value: string) => {
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <main className="flex-1 container mx-auto px-4 py-5 space-y-4">
        ...
        <div className="border border-steel-800 rounded bg-card/30 overflow-hidden">
          ...
          <div className="p-4 space-y-5">
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
              ...
              <div className="space-y-1">
                ...
                <Select value={level} onValueChange={(v) => setLevel(v ?? '88')}>
                  <SelectTrigger className="h-8 text-xs border-steel-700 bg-steel-900/50 font-mono"><SelectValue /></SelectTrigger>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1">
                <Label className="text-[10px] font-mono text-steel-500 uppercase tracking-wider">主属性类型</Label>
              ...
              <div className="space-y-1">
                <Label className="text-[10px] font-mono text-steel-500 uppercase tracking-wider">主属性值</Label>
            ...
            <div className="space-y-2">
              <Label className="text-[11px] font-mono text-steel-300 uppercase tracking-wider font-medium">副属性</Label>
              {substats.map((sub, i) => (
                <div key={i} className="grid grid-cols-3 gap-2 items-end">
                  <div className="space-y-1">
                    <Label className="text-[10px] font-mono text-steel-500">类型 {i + 1}</Label>
                    <Select value={sub.type as any} onValueChange={(v: any) => updateSubstat(i, 'type', v ?? '')}>
                      <SelectTrigger className="h-7 text-xs border-steel-700 bg-steel-900/50 font-mono"><SelectValue placeholder="选择属性" /></SelectTrigger>
                      <SelectContent className="border-steel-700 bg-steel-900">
                        {STAT_TYPES.map(s => (
                          <SelectItem key={s} value={s}>{StatTypeDisplay[s]}</SelectItem>
                        ))}
                  ...
                  <div className="space-y-1">
                    <Label className="text-[10px] font-mono text-steel-500">数值</Label>
                    <Input type="number" value={sub.value} onChange={e => updateSubstat(i, 'value', e.target.value)}
                      className="h-7 text-xs border-steel-700 bg-steel-900/50 font-mono" />
                  </div>
                  <div className="space-y-1">
                    <Label className="text-[10px] font-mono text-steel-500">强化次数</Label>
                    <Input type="number" value={sub.rolls} onChange={e => updateSubstat(i, 'rolls', e.target.value)} min={0} max={6}
                      className="h-7 text-xs border-steel-700 bg-steel-900/50 font-mono" />
                  </div>
                </div>
              ))}
        ...
        <div className="border border-steel-800 rounded bg-card/30 overflow-hidden">
          <div className="px-4 py-2.5 border-b border-steel-800 bg-steel-950/30">
            <span className="text-[11px] font-mono text-steel-400 uppercase tracking-wider font-medium">转换规则说明</span>
          </div>
          <div className="p-4 text-[11px] text-steel-400 space-y-1.5 font-mono">
            <p>• 只有强化次数 ≤ 3 的副属性可以转换</p>
            <p>• 88级装备转换上限与90级装备不同</p>
            <p>• 转换后的属性值受装备等级和强化次数限制</p>
            <p>• 每个副属性只能转换为该装备部位允许的属性类型</p>
